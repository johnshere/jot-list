...
export const replySuccess = <T>(reply: FastifyReply, data: T, status?: HttpStatus) => {
    return reply.code(status || HttpStatus.OK).send({ success: true, failed: false, data } satisfies SuccessResult<T>)
}
export const replyError = (reply: FastifyReply, message: string, status: HttpStatus | BizCode = HttpStatus.BAD_REQUEST) => {
